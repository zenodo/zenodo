...
GITHUB_ERROR_HANDLERS = [
    ...
    (jsonref.JsonRefError, "zenodo.modules.github.error_handlers.invalid_ref_error"),
    (Exception, "zenodo.modules.github.error_handlers.default_error"),
]
#   (
...
RECORDS_REST_FACETS = dict(
    records=dict(
        ...
        aggs=dict(
            ...
            publication_date=dict(
                date_histogram=dict(
                    field="publication_date", interval="1y", format="yyyy"
                )
            ),
        ),
        filters=dict(
            ...
            publication_date=range_filter(field="publication_date", format="yyyy"),
        ),
        post_filters=dict(
            ...
            related_identifiers_type=terms_filter("related.resource_type.type"),
            related_identifiers_subtype=terms_filter("related.resource_type.subtype"),
        ),
    )
)
...
STATS_QUERIES = {
    ...
    "record-view-all-versions": dict(
        ...
        params=dict(
            ...
            query_modifiers=[lambda query, **_: query.filter("term", is_parent=True)],
            required_filters=dict(
                conceptrecid="conceptrecid",
            ),
            metric_fields=dict(
