...
class DerivedMetadataSerializer(object):
    """Derived metadata serializer.

    Serializes the derived metadata types.
    """

    def serialize(self, pid, record, links_factory=None):
        """Include files for single record retrievals."""
        from zenodo.modules.deposit.views_rest import _metadata_file_key, \
            _mimetype_whitelist
        mimetype = request and request.args.get('mimetype')
        if not mimetype:
            return 'Mimetype not specified.'
        if mimetype not in _mimetype_whitelist():
            return '"{}" is not an acceptable mimetype.'.format(mimetype)
        ...
        if key in deposit.files:
            ...
            # TODO: encode?
            return result
        return 'No metadata available for this mimetype.'
