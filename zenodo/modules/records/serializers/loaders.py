...
def json_marshmallow_translator_loader(marshmallow_schema, translator):
    """Create a function that loads the json and translates it."""
    def json_marshmallow_loader_result():
        """Parse the request to create the dictionary."""
        translated_recrod = translator(request.get_json())
        return marshmallow_schema().dump(translated_recrod).data
    return json_marshmallow_loader_result
