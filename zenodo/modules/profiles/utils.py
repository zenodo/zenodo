...
def format_request_email_title(context):
    ...
    :rtype: str
    """
    template = current_app.config['ZENODO_PROFILES_EMAIL_TITLE_TEMPLATE']
...
def format_request_email_body(context):
    ...
    :rtype: str
    """
    template = current_app.config['ZENODO_PROFILES_EMAIL_BODY_TEMPLATE']
    return render_template_to_string(template, context)
...
def send_support_email(context):
    ...
    msg = Message(
        msg_title,
        sender=current_app.config['ZENODO_PROFILES_SENDER_EMAIL'],
...
def gravatar_url(email, size):
    """Get url of the Gravatar image of a user."""
    return ('https://www.gravatar.com/avatar/' + hashlib.md5(
        email.lower().encode('utf-8')
    ).hexdigest() + '?' + urllib.parse.urlencode({'s': str(size)}))


def user_orcid_id(user, orcid_id):
    """Give Orcid id of user."""
    if (orcid_id is None and user.remote_accounts):
        for account in user.remote_accounts:
            if account.client_id == current_app.config.get(
...
def show_profile(user):
    """Signal to show profile."""
    return (user and user.researcher_profile and user.profile
