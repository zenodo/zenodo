...
<div class="media htmlbrief">
    ...
    <div class="media-body">
        <span class="label label-info" data-toggle="tooltip" title="Upload date">{{ bfe_date(bfo, date_format='%d %B %Y') }}</span>
        ...
        {% if bfo.field('8560_y') %}<small class="muted">Uploaded by <a href="{{ url_for('yourmessages.write', sent_to_user_nicks=bfo.field('8560_y')) }}">{{bfo.field('8560_y')}}</a> on {{ bfe_creation_date(bfo, date_format="%d %M %Y") }}.</small>{% endif %}
    </div>
</div>